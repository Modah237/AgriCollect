...
export default function PaymentsPage() {
  ...
  const { data: batches, isLoading } = useQuery({
    queryKey: ['batches', user?.gicId],
    queryFn: () => api.get(`/payments/batches`).then((r) => r.data),
    refetchInterval: 10000, // polling toutes les 10s pour les batches en cours
  })

  const { data: batchDetail } = useQuery({
    queryKey: ['batch-detail', selectedBatch],
    queryFn: () => api.get(`/payments/batches/${selectedBatch}`).then((r) => r.data),
  ...
  const launchMutation = useMutation({
    mutationFn: () =>
      api.post('/payments/batches', {
        campaignId: campaign?.id,
        producerIds: producers?.map((p: any) => p.id) ?? [],
      }),
    onSuccess: () => {
              ...
              {batches.map((b: any) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 hover:bg-gray-50 transition-colors ${
                    selectedBatch === b.id ? 'bg-green-50 border-l-4 border-green-600' : ''
                  }`}
                >
